...
export default class CrawlerBase {
  ...
  async runX() {
    ...
    try {
      if (1 == 1) {
        ...
        await page.$eval('#code[name=code]', ($input, c) => $input.value = c, code);
  ...
  listMails = async (gmail: gmail_v1.Gmail) => {
    try {
  ...
  // 获取邮件内容
  getMessage = async (gmail: gmail_v1.Gmail, messageId: string) => {
    try {
