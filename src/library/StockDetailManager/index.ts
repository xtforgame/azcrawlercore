...
export default class StockNewsManager {
  ...
  async update(companyMap) {
    ...
    const existsRows2 = await sendQuery(`SELECT symbol_uid, year, quarter, score FROM company_factor_score WHERE factor_type = 'D01' AND score > 0 AND year = 2021 AND quarter = 1;`);
